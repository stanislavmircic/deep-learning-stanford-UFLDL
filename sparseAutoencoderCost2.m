...
for m=1:numberOfExamples
    propagatedDelta = [propagatedDelta (((W2')*delta2(:,m)) + (-(sparsityParam./meanRho) + (1-sparsityParam)./(1-meanRho)).*beta)];
end

delta1 = propagatedDelta.*(A1.*(1.-A1));


%calculate W2grad and b2grad 
for m=1:numberOfExamples
    
    for neuronIndex=1:visibleSize
        W2grad(neuronIndex,:) = W2grad(neuronIndex,:) + (A1(:,m).*delta2(neuronIndex,m))'; 
...
for m=1:numberOfExamples
    ...
    for neuronIndex=1:hiddenSize
        W1grad(neuronIndex,:) = W1grad(neuronIndex,:) + (data(:,m).*delta1(neuronIndex,m))'; 
